...
## WHAT IT IS
The shipped scorpius re-authored in the Star Store: 16 stars, 15 lines, 2 eyes; idle pinch, attack lash.
...
// READY TODAY (stars/edges/eyes/tint/eye/fx only — zero engine change) · objects 28 battle / 27 home
...
- Boot headless on the crisp-check.mjs pattern (tools/crisp-check.mjs:37-131) on ?rend=cv AND ?rend=gl, iPhone SE + iPhone 14 + iPad geometries: before load, set localStorage beta3.camproster to a 20-id roster with 'scorpius' in slot 0 (and clear beta3.campaign); tap NEW CAMPAIGN through the sign sheet; wait for game.scene.isActive('battle'); assert game.scene.getScene('battle').beast.id === 'scorpius'.
- assert beastC.list.length ≤ 40 sampled at N×40+700 ms (N = 16 stars; expected 28, glints excluded) and ≤ 30 in the home showcase (expected 27).
- zero Runtime.exceptionThrown over 20 s AND the frame counter still advances; one strike lands.
- `SS_BEAST_T(SS_BEASTS.scorpius)` returns a non-empty string (strings.js:1547 — SS_BEAST_T(b) takes the beast OBJECT and reads b.id, never an id string) and `Object.keys(SS_LANGS).every(l => l==='en' || (SS_STR[l].beast && SS_STR[l].beast.scorpius))` (or boot once per ?lang=xx).
