READY TODAY (PLAIN COPY — the game can draw this today; the waltz and the moon-bolts arrive with SS-SKY-01 (idle orbit→ripple)). Fire any time.
# SS-OPP-neptune — NEPTUNE · THE DEEP BLUE — a new STARSPELL opponent from the Star Store
lane: /Users/drbango/DrBango/beta3 · account: per the ring · effort: max · made in the Star Store 2026-08-25 by Skylar

## WHAT IT IS
THE DEEP BLUE as a boss lvl 3 opponent: 8 stars, 4 lines, 1 eye; idle ripple, attack charge.

## WHERE IT LIVES
- SS_BEASTS (data.js, at the end of the bosses block @150-216) — the block below, verbatim.
- SS_ACTS[2].bosses @239-265: append 'neptune' — Wyatt's seat to confirm.
...
- Boot headless on the crisp-check.mjs pattern (tools/crisp-check.mjs:37-131) on ?rend=cv AND ?rend=gl, iPhone SE + iPhone 14 + iPad geometries: before load, set localStorage beta3.camproster to a 20-id roster with 'neptune' in slot 0 (and clear beta3.campaign); tap NEW CAMPAIGN through the sign sheet; wait for game.scene.isActive('battle'); assert game.scene.getScene('battle').beast.id === 'neptune'.
- assert beastC.list.length ≤ 40 sampled at N×40+700 ms (N = 8 stars; expected 19, glints excluded) and ≤ 30 in the home showcase (expected 18).
- zero Runtime.exceptionThrown over 20 s AND the frame counter still advances; one strike lands.
- `SS_BEAST_T(SS_BEASTS.neptune)` returns a non-empty string (strings.js:1547 — SS_BEAST_T(b) takes the beast OBJECT and reads b.id, never an id string) and `Object.keys(SS_LANGS).every(l => l==='en' || (SS_STR[l].beast && SS_STR[l].beast.neptune))` (or boot once per ?lang=xx).
