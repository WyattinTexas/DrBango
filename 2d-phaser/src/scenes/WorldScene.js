...
class WorldScene extends Phaser.Scene {
  ...
  _updateBuffHUD() {
    ...
    if (buffs.length === 0) {
      this._buffHudText.setVisible(false);
      return;
    }
    let text = '';
  ...
  _checkStructureProximity() {
    ...
    const playerTX = Math.floor(G.x);
    const playerTY = Math.floor(G.y);
    const nearby = getStructuresNear(playerTX, playerTY, 3);

    // Show interaction hints for nearby structures
    for (const s of nearby) {
      const info = STRUCTURE_TYPES[s.type];
      if (!info) continue;

      // Garden: Shaman meditation
      if (s.type === 'garden' && typeof hasBuff === 'function' && typeof isApprentice === 'function') {
        if (isApprentice('shaman') && !hasBuff('meditation')) {
          // Show meditation hint (handled by existing NPC hint pattern)
        }
      }

      // Tavern: Scholar preparation
      if (s.type === 'tavern' && typeof isApprentice === 'function') {
        if (isApprentice('scholar') && !hasBuff('preparation')) {
