...
async function equipRaidItem(itemKey) {
  ...
  const ref = db.ref(`mp_modelc/users/${user.uid}/raidRunInventory`);
...
async function unequipRaidSlot(slot) {
  ...
  const ref = db.ref(`mp_modelc/users/${user.uid}/raidRunInventory/equipped/${slot}`);
  await ref.remove();
}
...
async function getRaidEquipped() {
  ...
  const snap = await db.ref(`mp_modelc/users/${user.uid}/raidRunInventory`).once('value');
  const inv = snap.val();
...
async function joinRaidQueue(raidId, team) {
  ...
  if (bossConfig.requiredBadge) {
    const userSnap = await db.ref(`mp_modelc/users/${user.uid}/raidBadges`).once('value');
  ...
  const queueRef = db.ref(`mp_modelc/raids/queue/${raidId}/${user.uid}`);
  await queueRef.set({
...
async function leaveRaidQueue(raidId) {
  const user = firebase.auth().currentUser;
  if (!user) return;
  await db.ref(`mp_modelc/raids/queue/${raidId}/${user.uid}`).remove();
...
function startQueueListener(raidId) {
  ...
  const queueRef = db.ref(`mp_modelc/raids/queue/${raidId}`);
  raidListeners['queue_' + raidId] = queueRef.on('value', async (snap) => {
    ...
    if (!queue) {
      ...
      const user = firebase.auth().currentUser;
      if (user && !currentRaid) {
        const activeSnap = await db.ref(`mp_modelc/users/${user.uid}/activeRaid`).once('value');
        const activeId = activeSnap.val();
        if (activeId) {
          const instSnap = await db.ref(`mp_modelc/raids/instances/${activeId}`).once('value');
...
async function startRaidManually(raidId) {
  const queueSnap = await db.ref(`mp_modelc/raids/queue/${raidId}`).once('value');
...
function stopQueueListener(raidId) {
  ...
  if (raidListeners[key]) {
    db.ref(`mp_modelc/raids/queue/${raidId}`).off('value', raidListeners[key]);
...
async function tryCreateRaidInstance(raidId, players) {
  ...
  // Create instance
  const instanceRef = db.ref('mp_modelc/raids/instances').push();
  ...
  const updates = {};
  players.forEach(p => {
    updates[`mp_modelc/users/${p.uid}/activeRaid`] = instanceId;
  });
...
function startActiveRaidListener() {
  ...
  db.ref(`mp_modelc/users/${user.uid}/activeRaid`).on('value', async (snap) => {
    ...
    if (justCompleted) {
      ...
      await db.ref(`mp_modelc/users/${user.uid}/activeRaid`).remove();
      return;
    }

    // Load into the raid
    const instSnap = await db.ref(`mp_modelc/raids/instances/${instanceId}`).once('value');
...
function enterRaidScreen(instanceId) {
  ...
  const instRef = db.ref(`mp_modelc/raids/instances/${instanceId}`);
        ...
        if (!window._raidWaitingRoomShown) {
          ...
          if (slot0 && user && slot0.uid === user.uid) {
            setTimeout(async () => {
              ...
              const ps = await db.ref(`mp_modelc/raids/instances/${instanceId}/fightPhase`).once('value');
              if (ps.val() === 'countdown') {
                db.ref(`mp_modelc/raids/instances/${instanceId}`).update({
...
async function startMyRaidFight(raidData) {
  ...
  // Update our status
  db.ref(`mp_modelc/raids/instances/${currentRaid.instanceId}/players/${currentIdx}/status`).set('fighting');
  ...
  if (user) {
    try {
      const invSnap = await db.ref(`mp_modelc/users/${user.uid}/raidRunInventory`).once('value');
...
async function drainBossHpPool(damage) {
  ...
  // Update Firebase
  await db.ref(`mp_modelc/raids/instances/${instanceId}`).update({
    bossCurrentHp: newHp,
...
async function endMyRaidFight(result) {
  ...
  // Update player stats
  await db.ref(`mp_modelc/raids/instances/${instanceId}/players/${slotIdx}`).update({
    status: 'done',
...
async function distributeRaidRewards(instanceId, bossDefeated, killingBlowUid) {
  ...
  for (const [slot, p] of Object.entries(players)) {
    ...
    if (points > 0) {
      updates[`mp_modelc/users/${p.uid}/raidPoints`] = firebase.database.ServerValue.increment(points);
    }

    // Update raid stats
    updates[`mp_modelc/users/${p.uid}/raidStats/raidsCompleted`] = firebase.database.ServerValue.increment(1);
    updates[`mp_modelc/users/${p.uid}/raidStats/totalBossDamage`] = firebase.database.ServerValue.increment(p.damageDealt || 0);

    if (p.uid === killingBlowUid) {
      updates[`mp_modelc/users/${p.uid}/raidStats/killingBlows`] = firebase.database.ServerValue.increment(1);
    }
    ...
    if (bossDefeated && p.status !== 'disconnected') {
      ...
      updates[`mp_modelc/raids/instances/${instanceId}/players/${slot}/lootRoll`] = loot.roll;
      updates[`mp_modelc/raids/instances/${instanceId}/players/${slot}/lootType`] = loot.rollType;
      updates[`mp_modelc/raids/instances/${instanceId}/players/${slot}/lootItem`] = loot.item ? loot.item.key : null;
      updates[`mp_modelc/raids/instances/${instanceId}/players/${slot}/lootItemName`] = loot.item ? loot.item.name : null;
      updates[`mp_modelc/raids/instances/${instanceId}/players/${slot}/lootItemIcon`] = loot.item ? loot.item.icon : null;

      // Save to player's raid run inventory (persists across bosses within this raid)
      const invRef = db.ref(`mp_modelc/users/${p.uid}/raidRunInventory`);
  ...
  updates[`mp_modelc/raids/instances/${instanceId}/status`] = 'complete';
  updates[`mp_modelc/raids/instances/${instanceId}/completedAt`] = firebase.database.ServerValue.TIMESTAMP;
  updates[`mp_modelc/raids/instances/${instanceId}/fightPhase`] = 'done';
  if (bossDefeated && killingBlowUid) {
    updates[`mp_modelc/raids/instances/${instanceId}/bossDefeatedBy`] = killingBlowUid;
  }
...
async function awardRaidBadge(uid, badgeKey) {
  const snap = await db.ref(`mp_modelc/users/${uid}/raidBadges`).once('value');
  const badges = snap.val() || [];

  if (!badges.includes(badgeKey)) {
    badges.push(badgeKey);
    await db.ref(`mp_modelc/users/${uid}/raidBadges`).set(badges);
    ...
    Object.entries(RAID_BADGES).forEach(([key, badge]) => {
      if (badge.requires && !badges.includes(key)) {
        if (badge.requires.every(req => badges.includes(req))) {
          badges.push(key);
          db.ref(`mp_modelc/users/${uid}/raidBadges`).set(badges);
        }
      }
    });
  }
...
function startHeartbeat(slotIdx) {
  ...
  heartbeatTimer = setInterval(() => {
    if (currentRaid) {
      db.ref(`mp_modelc/raids/instances/${currentRaid.instanceId}/players/${slotIdx}/lastHeartbeat`)
...
async function writeBattleSnapshot(snapshotData) {
  ...
  await db.ref(`mp_modelc/raids/instances/${instanceId}/battleState`).set(snapshot);
}
...
function cleanupRaid() {
  ...
  if (currentRaid?.instanceId) {
    const instRef = db.ref(`mp_modelc/raids/instances/${currentRaid.instanceId}`);
...
async function cleanupStaleRaids() {
  ...
  const instSnap = await db.ref('mp_modelc/raids/instances').orderByChild('status').equalTo('active').once('value');
  const instances = instSnap.val() || {};
  for (const [id, inst] of Object.entries(instances)) {
    if (inst.startedAt && (serverNow - inst.startedAt) > RAID_CONFIG.STALE_INSTANCE_MS) {
      await db.ref(`mp_modelc/raids/instances/${id}/status`).set('abandoned');
      // Clear activeRaid for all players
      if (inst.players) {
        const updates = {};
        Object.values(inst.players).forEach(p => {
          updates[`mp_modelc/users/${p.uid}/activeRaid`] = null;
        });
        await db.ref().update(updates);
      }
    }
  ...
  for (const raidId of allBosses) {
    const qSnap = await db.ref(`mp_modelc/raids/queue/${raidId}`).once('value');
    ...
    for (const [uid, entry] of Object.entries(queue)) {
      if (entry.joinedAt && (serverNow - entry.joinedAt) > RAID_CONFIG.STALE_QUEUE_MS) {
        await db.ref(`mp_modelc/raids/queue/${raidId}/${uid}`).remove();
      }
    }
  }
}
...
async function initRaidSystem() {
  ...
  const user = firebase.auth().currentUser;
  if (user) {
    const arSnap = await db.ref(`mp_modelc/users/${user.uid}/activeRaid`).once('value');
    const activeId = arSnap.val();
    if (activeId) {
      const instSnap = await db.ref(`mp_modelc/raids/instances/${activeId}/status`).once('value');
