...
function showRaidLobby() {
  ...
  // Get user's badges
  db.ref(`mp_modelc/users/${user.uid}/raidBadges`).once('value').then(snap => {
...
function openEquipPicker(slot) {
  ...
  db.ref(`mp_modelc/users/${user.uid}/raidRunInventory`).once('value').then(snap => {
    const inv = snap.val();
    const items = inv?.items || [];
    const equipped = inv?.equipped || { head: null, weapon: null, accessory: null };
    const slotDef = EQUIP_SLOTS[slot];
...
function checkForActiveRaids() {
  db.ref('mp_modelc/raids/instances').orderByChild('status').equalTo('active').limitToFirst(1).on('value', snap => {
...
function spectateRaid(instanceId) {
  ...
  const user = firebase.auth().currentUser;
  if (user) {
    db.ref(`mp_modelc/raids/instances/${instanceId}/spectators/${user.uid}`).set({
...
function launchFromWaitingRoom() {
  ...
  if (currentRaid && currentRaid.instanceId) {
    db.ref(`mp_modelc/raids/instances/${currentRaid.instanceId}`).update({
      status: 'active',
...
function startRaidChatListener(instanceId) {
  stopRaidChatListener();
  const chatRef = db.ref(`mp_modelc/raids/instances/${instanceId}/chat`);
...
function stopRaidChatListener() {
  if (_wrChatListener && _wrInstanceId) {
    db.ref(`mp_modelc/raids/instances/${_wrInstanceId}/chat`).off('child_added', _wrChatListener);
...
function pushRaidChatMessage(instanceId, displayName, message, isSystem) {
  db.ref(`mp_modelc/raids/instances/${instanceId}/chat`).push({
...
function startSpectatorBattleListener(instanceId) {
  ...
  const ref = db.ref(`mp_modelc/raids/instances/${instanceId}/battleState`);
...
function stopSpectatorBattleListener() {
  if (_specBattleListener && (_wrInstanceId || currentRaid?.instanceId)) {
    ...
    db.ref(`mp_modelc/raids/instances/${id}/battleState`).off('value', _specBattleListener);
