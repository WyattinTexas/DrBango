...
function showRaidShop() {
  ...
  db.ref(`mp_modelc/users/${user.uid}`).once('value').then(snap => {
...
function renderRaidShop(points, collection, badges) {
  ...
  const user = firebase.auth().currentUser;
  if (user) {
    db.ref(`mp_modelc/users/${user.uid}/raidStats`).once('value').then(snap => {
...
async function purchaseRaidItem(itemId) {
  ...
  const userRef = db.ref(`mp_modelc/users/${user.uid}`);
  ...
  const collSnap = await db.ref(`mp_modelc/users/${user.uid}/collection`).once('value');
  const collection = collSnap.val() || [];
  cards.forEach(c => {
    if (!collection.includes(c.id)) collection.push(c.id);
  });
  await db.ref(`mp_modelc/users/${user.uid}/collection`).set(collection);
...
function updateRaidPointsDisplay() {
  ...
  db.ref(`mp_modelc/users/${user.uid}/raidPoints`).on('value', snap => {
